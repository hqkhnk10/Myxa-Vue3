        ...
        <div class="radio__container">
          ...
          <div class="radio__container">
            ...
            <input
              ...
              :checked="true"
            />
            <span class="radio__checkmark-disabled"></span>
          </div>
      </div>
<div v-if="uploading" class="uploading-container">
    <div
      class="drop-area"
      :class="{ 'drop-hover': isDropHover, 'cursor-pointer': uploadedFiles.length == 0, 'border-solid' : uploadedFiles.length >0 }"
    ...
    <misa-loading v-model="loading"></misa-loading>
      <div class="drop-area-mid" v-if="uploadedFiles.length == 0">Drag and drop files here</div>
      <div v-if="uploadedFiles.length">
        <div v-for="(file, index) in uploadedFiles" :key="index" class="border-bottom">
          <div class="file-info">
            <div class="file-name">{{ file.name }}</div>
            <div class="file-size">{{ formatFileSize(file.size) }}</div>
            <div class="flex"><div class="file-valid"></div>Hợp lệ</div>
            <misa-button type="link" @click.stop="openFileInput">Change File</misa-button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="sheet-selection" v-if="worksheets.length">
      ...
      <misa-combobox v-model="selectedSheet" :options="worksheets"></misa-combobox>
      </div>
      <div>
        <h3>Header:</h3>
  ...
  <div class="upload__button">
      <misa-button type="primary-border" @click="generateExcelAndUpload">Tải tệp mẫu</misa-button>
      <div class="flex gap-12px">
        <misa-button type="secondary" @click="cancel">Hủy</misa-button>
        <misa-button type="secondary" v-if="!uploading" @click="back">Quay lại</misa-button>
        <misa-button @click="uploadFile">Tiếp tục</misa-button>
      </div>
    </div>
  </div>
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        const data = new Uint8Array(e.target.result);
        const workbook = XLSX.read(data, { type: 'array' });
        const sheetName = this.worksheets.find(sheet=> sheet.value == this.selectedSheet).label;
    ...
    generateExcelAndUpload() {
      ...
      const file = new File([new Uint8Array(excelBuffer)], filename, { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const fileList = [file]
